...
const PhoneInput = ({
  ...
  // Create a mapping from country code to label for easy lookup
  const countryCodes = countries.reduce(
    (acc, { code, label }) => ({ ...acc, [code]: label }),
    {}
  );

  // Handles change in the country code dropdown
  const handleCountryChange = (e) => {
    const newCountry = e.target.value;
    setSelectedCountry(newCountry);
    // Update phone number to the selected country's code/label
    setPhoneNumber(countryCodes[newCountry]);
    if (onChange) {
      onChange(countryCodes[newCountry]); // Notify parent component of the change
    }
  };
  ...
  const handlePhoneNumberChange = (e) => {
    ...
    if (onChange) {
      onChange(newPhoneNumber); // Notify parent component of the change
    }
  };

  return (
    <div className="relative flex">
      {/* Dropdown for country code when positioned at the start */}
      ...
      <input
        ...
        className={`dark:bg-dark-900 h-11 w-full ${
          selectPosition === "start" ? "pl-[84px]" : "pr-[84px]" // Adjust padding based on dropdown position
      ...
      {/* Dropdown for country code when positioned at the end */}
      {selectPosition === "end" && (
