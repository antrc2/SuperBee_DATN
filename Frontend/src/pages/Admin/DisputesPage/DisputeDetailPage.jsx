...
import { Link, useParams, useNavigate } from "react-router-dom"; // Giữ nguyên import này
...
export default function DisputeDetailPage() {
  // THAY ĐỔI: Đổi tên router -> navigate cho đúng convention
  const navigate = useNavigate();
  const { id } = useParams(); // useParams giữ nguyên, không đổi
  ...
  useEffect(() => {
    ...
    const fetchDisputeDetail = async () => {
      ...
      try {
        ...
        setNewStatus(
          response.data.status > 0 ? String(response.data.status) : "1"
        );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        // THAY ĐỔI: Dùng navigate() thay cho router.push()
  ...
  let attachments = [];
  try {
    if (dispute.attachments) attachments = JSON.parse(dispute.attachments);
  } catch (e) {
        ...
        >
          ...
          <div className="lg:col-span-2 lg:border-l lg:pl-8 border-gray-200 dark:border-gray-700 space-y-4">
            ...
            <div>
              ...
              >
                {Object.entries(DISPUTE_STATUS_OPTIONS).map(([key, value]) => (
