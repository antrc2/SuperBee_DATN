...
import api from '../../../utils/http'; // <-- IMPORT AXIOS INSTANCE CỦA BẠN TẠI ĐÂY
...
const authToken = sessionStorage.getItem("access_token");
  ...
  const froalaConfig = {
    ...
    // Theme (tùy chọn, cần import CSS theme tương ứng)
    // theme: 'gray',
    // Nút toolbar để có giao diện gần giống Word
    toolbarButtons: {
      moreText: {
        ...
        buttonsVisible: 6 // Số nút hiển thị trực tiếp
      },
      moreParagraph: {
      ...
      moreMisc: {
        buttons: ['quote', 'codeView', 'fullscreen', 'help', 'selectAll', 'print', 'getPDF', 'html', 'save'], // 'save' nếu có plugin save
        buttonsVisible: 3
      }
    },
    // Các tùy chọn nâng cao cho việc chèn ảnh
   imageUpload: true,
  ...
  useEffect(() => {
    const getCategories = async () => {
      try {
        ...
        setCategories(response.data?.data); // Giả sử API trả về { data: [...] }
      } catch (error) {
  ...
  const handleSubmitPost = async (e) => {
    ...
    try {
      ...
      formData.append('content', postContent); // Gửi nội dung HTML từ Froala Editor
      formData.append('category_id', selectedCategoryId || ''); // Gửi '' nếu không chọn danh mục

      // TODO: Thay thế bằng ID tác giả thực tế từ session/auth của bạn
      ...
      if (err.response?.data?.errors) {
        ...
        for (const key in err.response.data.errors) {
          backendErrors[key] = err.response.data.errors[key][0]; // Lấy tin nhắn lỗi đầu tiên
        }
      ...
      <form onSubmit={handleSubmitPost} className="space-y-6">
        ...
        <div>
          ...
          <FroalaEditorComponent
            tag='textarea' // Sử dụng textarea làm phần tử gốc
            config={froalaConfig}
            model={postContent} // Gán state postContent vào model của Froala
            onModelChange={setPostContent} // Cập nhật state postContent khi nội dung thay đổi
          />
            ...
            <select
              ...
              onChange={(e) => {
                if (e.target.value === 'new') {
                  ...
                  setSelectedCategoryId(''); // Xóa chọn danh mục cũ khi muốn tạo mới
                } else {
                  setSelectedCategoryId(e.target.value);
                  setErrors((prev) => ({ ...prev, category: '' })); // Xóa lỗi danh mục
                }
              }}
