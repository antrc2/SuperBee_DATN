...
const DiscountModal = ({
  ...
  console.log("🚀 ~ DiscountModal ~ discounts:", discounts);
                ...
                >
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1">
                      {/* --- KHU VỰC 1: ƯU ĐÃI CHÍNH --- */}
                      {/* Hiển thị giá trị giảm giá làm tiêu đề, nổi bật hơn */}
                      ...
                      {/* Hiển thị mô tả riêng biệt, luôn luôn hiện nếu có */}
                      <p className="text-sm text-secondary mb-3 pl-9">
                        {discount.description}
                      </p>

                      {/* --- KHU VỰC 2: MÃ GIẢM GIÁ --- */}
                      <div className="mb-3 flex items-center gap-3">
                        <span className="text-xs text-secondary/90">Mã: </span>
                        <span className="font-mono bg-tertiary px-2 py-1 rounded-md text-sm text-primary font-semibold">
                          {discount.code}
                        </span>
                        {appliedPromotionCode === discount.code && (
                      ...
                      {/* --- KHU VỰC 3: ĐIỀU KIỆN ÁP DỤNG --- */}
                      <div className="text-xs text-secondary/80 space-y-1 border-t border-themed pt-2">
                        <p>
                          HSD:{" "}
                    ...
                    <button
                      ...
                      className="action-buttons action-button-primary text-sm !py-2 !px-4 self-center disabled:!bg-tertiary disabled:!text-primary/80 disabled:cursor-not-allowed disabled:filter-none disabled:transform-none w-fit"
                    >
                      {appliedPromotionCode === discount.code
                        ? "Đã chọn"
                        : "Dùng ngay"}
      ...
      try {
        ...
        if (response.data?.status) {
          const items = response.data.carts
            ...
            .map((item) => {
              console.log("🚀 ~ fetchCheckoutData ~ item:", item);
              return {
                id: item.id,
                ...
                <div className="mb-4">
                  ...
                  >
                    <input
                      ...
                      className="h-4 w-4 mr-2 mt-0.5 rounded"
                    />
                    <span>
