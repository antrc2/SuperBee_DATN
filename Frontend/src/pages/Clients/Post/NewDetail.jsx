...
// Helper function to build a nested comment structure (keep as is)
...
export default function NewDetail() {
    ...
    // Function to transform image tags with video sources to video tags
    const transformVideoTags = (htmlContent) => {
        if (!htmlContent) return '';
        // This regex looks for <img> tags where the src ends with .mp4, .webm, .ogg
        // It captures the full src attribute.
        const videoRegex = /<img\s[^>]*src="([^"]+\.(?:mp4|webm|ogg))"[^>]*alt="([^"]*)"[^>]*>/gi;

        return htmlContent.replace(videoRegex, (match, src, alt) => {
            // Construct a <video> tag. Adding controls, preload="metadata", and className for styling.
            // You can add 'autoplay', 'loop', 'muted' if needed.
            return `<video controls preload="metadata" class="w-full rounded-lg my-4" alt="${alt}">
    ...
    // Effect to fetch news details and related news
    useEffect(() => {
        const fetchNewsDetail = async () => {
            try {
                ...
                if (newsData.content) {
                    ...
                    const finalContent = transformVideoTags(markdownToHtml); // Apply transformation
    ...
    // Callback function to fetch comments (keep as is)
    ...
    // Effect to fetch comments when news.id is available (keep as is)
    useEffect(() => {
        if (news?.id) {
    ...
    // handleAddComment and handleRootCommentSubmit (keep as is)
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 sm:p-6">
            <div className="flex flex-col lg:flex-row gap-6">
                ...
                {/* Related News Sidebar (keep as is) */}
