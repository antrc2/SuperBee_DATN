...
const ReviewWeb = () => {
  ...
  const nextSlide = () => {
    setCurrentIndex((prev) =>
      prev + 1 >= Math.ceil(reviews.length / getItemsPerSlide()) ? 0 : prev + 1
    );
  };
  ...
  const prevSlide = () => {
    setCurrentIndex((prev) =>
      prev - 1 < 0
        ? Math.ceil(reviews.length / getItemsPerSlide()) - 1
        : prev - 1
    );
  };
      ...
      <div className="py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="bg-card rounded-xl p-6 border-themed animate-pulse"
            >
    ...
    <div className="py-8 max-w-screen-xl mx-auto">
      ...
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-primary mb-2">
          Khách hàng nói gì về SHOPVPS.VN?
        </h2>
        <p className="text-secondary">
          Chia sẻ trải nghiệm thực tế từ cộng đồng người dùng
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 px-8">
          {visibleReviews.map((review) => (
            <div
              key={review.id}
              className="bg-card rounded-xl p-6 border border-themed hover:border-primary transition-colors shadow-sm"
            >
              {/* User Info */}
              <div className="flex items-center space-x-3 mb-4">
