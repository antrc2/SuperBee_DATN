...
class OrderController extends Controller
{
    ...
    public function Order(Request $request)
    {
        try {
            ...
            if ($request->total_price < $discount->min_discount_amount) {
                return response()->json([
                    "status" => false,
                    "message" => "Đơn hàng của bạn không đủ điều kiện sử dụng mã giảm giá này."
                ], 400);
            }
            if ($request->total_price < $discount->max_discount_amount) {
                $total_price = $request->total_price - $request->total_price * $discount->discount_amount / 100;
            } else {
                $total_price = $request->total_price - $discount->max_discount_amount * $discount->discount_amount / 100;
            }
            $order = Order::create([
                "user_id" => $userId,
                "web_id" => $webId,
